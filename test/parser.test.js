...
describe('MagicByteLengthParser', () => {
    it('transforms data by magic byte', () => {
        ...
        expect(fn.mock.calls[0][0]).toEqual(Buffer.from([0x32, 0x01, 0x88]))
        expect(fn.mock.calls[1][0]).toEqual(Buffer.from([0x32, 0x03, 0xff, 0x32, 0xff]))
        expect(fn.mock.calls[2][0]).toEqual(Buffer.from([0x32, 0x02, 0xaa, 0xab]))
    })
    it('handles zero length indicators', () => {
        ...
        expect(fn.mock.calls[0][0]).toEqual(Buffer.from([0x11, 0x00]))
        expect(fn.mock.calls[1][0]).toEqual(Buffer.concat([Buffer.from([0x11, 0x11]), Buffer.alloc(0x11).fill(0xff)]))
    })
    it('flushes on end', () => {
